...
// Memoized Components
const MemoizedHeader = React.memo(({ onSave, onUndo, onRedo, onExport, onImport, onCollaborate, onShowHistory }: {
  onSave: () => void,
  onUndo: () => void,
  onRedo: () => void,
  onExport: () => void,
...
const MemoizedToolbar = React.memo(({ tool, setTool, onEyeDropper }: {
  ...
  <div className="flex flex-col items-center p-2 border-r bg-white space-y-2">
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
...
const Canvas = React.memo(({
  ...
  }, [tool, shapes, zoom, setSelectedShapes, activeLayer, setShapes])
          ...
          shape.layer === activeLayer && (
            <div
              ...
              style={{
                ...
                border: selectedShapes.includes(shape.id) ? '2px solid blue' : 'none',
                display: 'flex',
                alignItems: 'center',
            ...
            >
              ...
              {selectedShapes.includes(shape.id) && !shape.locked && (
                <>
                  ...
                  />
                  <div
                    className="absolute w-5 h-5 bg-white border border-blue-500 rounded-full cursor-move"
...
const Minimap = React.memo(({ shapes, connections, zoom, activeLayer }: { shapes: Shape[], connections: Connection[], zoom: number, activeLayer: number }) => {
  ...
  return (
    <div className="absolute bottom-4 right-4 border border-gray-300 bg-white p-2">
...
const HistoryPanel = React.memo(({ history, currentIndex, onRestoreHistory }: {
  ...
  <div className="absolute top-16 right-4 w-64 bg-white border border-gray-300 rounded-lg shadow-lg p-4">
    <h3 className="text-lg font-semibold mb-2">History</h3>
    <ScrollArea className="h-64">
      {history.map((entry, index) => (
        <div
          key={index}
          className={`p-2 cursor-pointer ${index === currentIndex ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
...
export default function Component() {
  ...
  }, [handleUndo, handleRedo, handleSave, handleEyeDropper, shortcuts])
